# Makryvelios Research Analytics & Econometrics Command Centre v5.2.1
...
Version 5.2.1 includes a complete documentation set under `documentation/`:
...
- `tests/app_smoke.py` — render coverage for all eighteen modules.
...
This ZIP retains the folder name `makryvelios_dashboard_v2` for compatibility, but the software inside is version 5.2.1. Replace the old repository folder with the new one and keep the Streamlit main-file path as `makryvelios_dashboard_v2/app.py`. Streamlit rebuilds automatically after the GitHub commit.
